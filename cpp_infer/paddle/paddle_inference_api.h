...
using PPConfig = paddle::AnalysisConfig;
...
class PD_INFER_DECL Predictor {
 public:
  ...
  ///
  explicit Predictor(const PPConfig& config);
...
class PD_INFER_DECL PredictorPool {
 public:
  ...
  explicit PredictorPool(const PPConfig& config, size_t size = 1);
